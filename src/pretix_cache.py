...
class PretixCache:
    ...
    def refresh(self):
        ...
        for raw_order in raw_orders:
            ...
            if order_info is not None:
                discord_id, product, programs = order_info
                orders[discord_id] = (product, programs)
        self._orders = orders


    def get_user_info(self, name: str, discriminator: str):
        key = name + '#' + discriminator
        return self._orders[key] if key in self._orders else None
    ...
    def _extract_order_information(self, raw_order: Dict[str, any]) -> Optional[Tuple[str, Products, List[Programs]]]:
        ...
        if discord_id is None:
            return None
        return discord_id, product, programs
