...
class PretixApi:
    ...
    def get_orders(self, organizer: str, event_name: str, page: int = 1) -> Tuple[List[Dict[str, any]], Optional[int], Optional[int]]:
        result = requests.get(f'{self._api_url}/organizers/{organizer}/events/{event_name}/orderpositions/', headers=self._auth_headers).json()
        ...
        return result['results'], next_page, previous_page
